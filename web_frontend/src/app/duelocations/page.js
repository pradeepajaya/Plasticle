...
export default function DueLocationsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const urgentData = await checkUrgentAllocations();
      if (urgentData.data?.isUrgent) {
        setShowAlert(true);
      }
    };
  ...
  const handleGotIt = async () => {
    try {
      await notifyAllCollectors("Extra bin collection available near you. Do you accept?");
      toast.success("Collectors notified!");
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-green-400 via-white to-green-300 dark:from-green-800 dark:via-gray-900 dark:to-black text-black dark:text-white">
      <div className="p-6">
        {showAlert && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-md w-full text-center">
              <h2 className="text-xl font-bold text-red-600 mb-2">⚠️ Urgent Alert</h2>
              <p className="text-gray-800 dark:text-gray-200 mb-4">
                More than half of the bins are full. Immediate allocation is required.
              </p>
              <button
                onClick={handleGotIt}
                            ...
                            <div className="mt-3 flex flex-col sm:flex-row sm:items-center gap-3">
                              ...
                              >
                                ...
                                {collectors.length === 0 ? (
                                  ...
                                  collectors
                                    .filter((c) => {
                                      const prefersThisBin =
                                        Array.isArray(c.preferredBins) &&
                                        c.preferredBins.some(
                                          (b) => b._id?.toString() === binObjectId?.toString()
                                        );
                                      const hasNoPrefs =
                                        !Array.isArray(c.preferredBins) ||
                                    ...
                                    .map((c) => (
                                      <option key={c._id} value={c._id}>
                                        {`${c.userId?.nickname || 'Collector'} (${c._id.slice(
                                          -4
