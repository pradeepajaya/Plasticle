...
const updateBinCollectionStatus = async (req, res) => {
  try {
    const { binId, collected } = req.body;

    if (!binId || typeof collected !== 'boolean') {
    ...
    res.status(200).json({ message: "Bin collection status updated", bin });
  } catch (error) {
    console.error("Error updating bin collected status:", error);
    res.status(500).json({ message: "Server error" });
  }
};
